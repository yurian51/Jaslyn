...
function normalizeMac(mac) {
  ...
  if (!/^[0-9a-f]{12}$/.test(value)) {
    throw new TypeError("macAddress must contain exactly 12 hexadecimal characters");
  }
...
function normalizeCurrency(currency) {
  ...
  if (!SUPPORTED_CURRENCIES.has(value)) {
    throw new TypeError(`Unsupported currency: ${value || "empty"}`);
  }
  return value;
}
...
export function normalizeClientIdentity(identity) {
  ...
  if (!macAddress && !username && !ipAddress) {
    throw new TypeError("client identity requires macAddress, username, or ipAddress");
  }
...
export class WifiBillingEngine {
  ...
  constructor({ clock = () => Date.now(), idFactory = () => crypto.randomUUID() } = {}) {
    this.#clock = clock;
  ...
  getPlan(planId) {
    return this.#plans.get(String(planId)) ?? null;
  }
  ...
  startSession({ planId, client, startedAt = this.#clock() }) {
    ...
    const activeForClient = [...this.#sessions.values()].filter(
      (session) => session.status === "active" && sameIdentity(session.client, identity)
    );
    if (activeForClient.length >= plan.simultaneousDevices) {
      throw new Error("SIMULTANEOUS_DEVICE_LIMIT_REACHED");
    }
  ...
  recordUsage(sessionId, { uploadBytes = 0, downloadBytes = 0, at = this.#clock() } = {}) {
    ...
    return snapshotSession(session, this.#plans.get(session.planId));
  }
  ...
  closeSession(sessionId, { endedAt = this.#clock() } = {}) {
    ...
    session.chargedMinor = calculateCharge(this.#plans.get(session.planId), session.startedAt, endedAt, session.usage);
    return snapshotSession(session, this.#plans.get(session.planId));
  }
  ...
  listSessions({ status } = {}) {
    return [...this.#sessions.values()]
      ...
      .map((session) => snapshotSession(session, this.#plans.get(session.planId)));
  }
}
...
export class NetworkAdapterRegistry {
  ...
  register({ protocol, capabilities = [], adapter }) {
    ...
    if (typeof adapter.health !== "function" || typeof adapter.enforcePolicy !== "function") {
      throw new TypeError("adapter must expose health() and enforcePolicy()");
    }
  ...
  get(protocol) {
    return this.#adapters.get(String(protocol ?? "").trim().toUpperCase()) ?? null;
  }

  list() {
    return [...this.#adapters.values()].map(({ protocol, capabilities }) => ({ protocol, capabilities: [...capabilities] }));
  }
}
...
export function calculateCharge(plan, startedAt, endedAt, usage = { uploadBytes: 0, downloadBytes: 0 }) {
  ...
  if (!Number.isFinite(durationMs)) throw new TypeError("session timestamps must be finite");
  const durationSeconds = durationMs / 1000;
  if (durationSeconds === 0) return 0;
  if (normalized.dataLimitBytes != null) {
    const consumed = assertNonNegative(usage.uploadBytes, "usage.uploadBytes") + assertNonNegative(usage.downloadBytes, "usage.downloadBytes");
    if (consumed > normalized.dataLimitBytes) return normalized.priceMinor;
  }
  return normalized.priceMinor;
}
