...
export function normalizePaymentEvent(input) {
  if (!input?.provider || !input?.providerTransactionId || !input?.customerId || !input?.amountMinor || !input?.currency) throw new Error("provider, providerTransactionId, customerId, amountMinor and currency are required");
  ...
  return { provider: String(input.provider), providerTransactionId: String(input.providerTransactionId), customerId: String(input.customerId), subscriptionId: input.subscriptionId ? String(input.subscriptionId) : null, amountMinor, currency, status, providerReference: input.providerReference ? String(input.providerReference) : null, receivedAt: input.receivedAt ? new Date(input.receivedAt).toISOString() : new Date().toISOString() };
}
    ...
    const result = await client.query(
      `insert into net_payments (organization_id, customer_id, subscription_id, provider, provider_transaction_id, amount_minor, currency, status, provider_reference, received_at, verified_at)
       ...
       on conflict (provider, provider_transaction_id) do update set
         ...
         updated_at=now()
       returning id, provider, provider_transaction_id, amount_minor, currency, status, received_at, verified_at`,
      [input.organizationId, payment.customerId, payment.subscriptionId, payment.provider, payment.providerTransactionId, payment.amountMinor, payment.currency, payment.status, payment.providerReference, payment.receivedAt]
    );
