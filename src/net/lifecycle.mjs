...
function assertActivationPayment(payment, plan) {
  if (!VERIFIABLE_PAYMENT_STATES.has(payment.status)) {
    throw new Error(`Payment ${payment.id} is not verified or settled`);
  }
  if (payment.amount_minor !== plan.price_minor) {
    throw new Error(`Payment amount mismatch for plan ${plan.id}`);
  }
  if (payment.currency !== plan.currency) {
    throw new Error(`Payment currency mismatch for plan ${plan.id}`);
  }
}
  ...
  return withTransaction(async (client) => {
    ...
    const updatedSubscription = await client.query(
      `update net_subscriptions
       set status = 'ACTIVE',
           ...
           updated_at = now()
       where id = $1
       returning id, customer_id, plan_id, status, starts_at, expires_at, entitlement_version`,
      [subscription.id, activationTime, expiresAt, nextVersion],
    );

    const policy = compileNetworkPolicy({
      id: subscription.plan_id,
      name: subscription.plan_name,
      downloadMbps: Number(subscription.download_mbps),
      uploadMbps: Number(subscription.upload_mbps),
      quotaBytes: subscription.quota_bytes == null ? null : Number(subscription.quota_bytes),
      validitySeconds: Number(subscription.validity_seconds),
      deviceLimit: Number(subscription.device_limit),
    });
    ...
    const authorization = await client.query(
      `insert into net_authorizations
       ...
       values ($1,$2,$3,'PENDING',$4::jsonb,$5,'jaslyn-billing',$6::timestamptz,$7::jsonb)
      ...
      [
        ...
        nextVersion,
        activationTime,
        JSON.stringify({ reason: "PAYMENT_VERIFIED", paymentId: payment.id, networkAction: "PENDING" }),
      ],
    );
    ...
    const event = await client.query(
      ...
      [
        ...
        correlationId,
        nextVersion,
        JSON.stringify({ paymentId: payment.id, authorizationId: authorization.rows[0].id, expiresAt }),
      ],
    );
    ...
    await client.query(
      ...
      [
        ...
        subscription.id,
        correlationId,
        JSON.stringify({ status: subscription.status, expiresAt: subscription.expires_at }),
        JSON.stringify({ status: "ACTIVE", expiresAt, authorizationState: "PENDING" }),
      ],
    );
    ...
    return {
      ...
      event: event.rows[0],
      networkState: "PENDING_NETWORK_APPLY",
    };
  });
}
