...
export async function POST(request) {
  if (!hasSecret(request, "JASLYN_NET_COMMAND_KEY", "x-jaslyn-net-key")) return NextResponse.json({ ok: false, error: "Network command authorization is not configured or is invalid." }, { status: 401 });
  ...
  try {
    ...
    return NextResponse.json({ ok: verified, command: "DISCONNECT_USER", target: parsed.data.id, reason: parsed.data.reason ?? "operator_request", provider: "mikrotik-rest", result, verified, verificationAttempts }, { status: verified ? 200 : 409 });
  } catch (error) {
